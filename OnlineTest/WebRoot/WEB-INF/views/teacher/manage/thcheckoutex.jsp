...
<html lang="en">
	...
	<body>
		...
		<div id="exam_main_panle">
			...
			<div id="tools">
				<a href="${pageContext.request.contextPath }/teacher/manage/checkoutEPScore?eid=${PageSPD.getContent()[0].getStuPaper().getePaper().getEid() }">返回上一页</a>
				...
				<form>
					...
					<div id="radio">
						...
						<p id="r1">
							...
							<span class="type">
								单选题  【得分${pspd.getScore() }分】【学生的答案：${pspd.getEdanswer() }】  【正确答案：
					...
					<div id="multiple">
						...
						<p id="m1">
							...
							${pspd.getqItem().getQiname()}?(${pspd.getqItem().getQiscore()}分)&nbsp;
							<span class="type">
								多选题   【得分${pspd.getScore() }分】【学生的答案：${pspd.getEdanswer() }】【正确答案：
								<c:forEach items="${answerPageSPD }" var="an">
								<%-- ${an.getItem().getQiid()}--
								${pspd.getqItem().getQiid()}== --%>
								<c:if test="${an.getItem().getQiid() == pspd.getqItem().getQiid()}">
					...
					<div id="judge">
						...
						<p>
							...
							${pspd.getqItem().getQiname()}?(${pspd.getqItem().getQiscore()}分)&nbsp;
							<span class="type">
								判断题  【得分${pspd.getScore() }分】【学生的答案：${pspd.getEdanswer() }】【正确答案：
								<c:forEach items="${answerPageSPD }" var="an">
								<%-- ${an.getItem().getQiid()}--
								${pspd.getqItem().getQiid()}== --%>
								<c:if test="${an.getItem().getQiid() == pspd.getqItem().getQiid()}">
